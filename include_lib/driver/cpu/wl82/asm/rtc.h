...
enum sel_rtc_clk {
    RTC_CLK_SEL_INSIDE = 0,
    RTC_CLK_SEL_OUTSIDE,
};

enum sel_rtcvdd {
    RTCVDD_SUPPLY_OUTSIDE = 0,
    RTCVDD_SUPPLY_BOUND_INSIDE_VDDIO,
    RTCVDD_FLOAT,
};

enum sel_rtc_power_res {
    RES_48K = 0,
    RES_24K,
    RES_12K,
    RES_6K,
...
struct rtc_init_config {
    u8 rtc_clk_sel;
    u8 rtc_power_sel;
    u8 rtc_power_res_sel;
};
...
//第一个1使能闹钟唤醒,0为关闭闹钟唤醒后面两个参数无意义  中间参数为唤醒时间s, 1为需要关机 0为不需要关机
