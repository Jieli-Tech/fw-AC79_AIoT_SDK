...
extern const char *get_dui_auth_product_key(void);
extern const char *get_dui_auth_product_id(void);
extern const char *get_dui_auth_product_secret(void);
extern const char *get_dui_auth_deviceid(void);

static void init_dui_para(struct dui_para *para)
{
//使用白名单
#if 1
    strcpy(para->param.productKey, get_dui_auth_product_key());
    strcpy(para->param.productId, get_dui_auth_product_id());
    strcpy(para->param.ProductSecret, get_dui_auth_product_secret());
    strcpy(para->param.deviceID, get_dui_auth_deviceid());
//不使用白名单
#else
