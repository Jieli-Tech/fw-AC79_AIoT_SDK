...
#define SAVE_BUF_SIZE   (5*1024)//网络卡的时候这个buf需要改大
...
struct net_audio_hdl {
    ...
    int play_task_pid;         //music_task句柄
...
static int net_audio_buf_write(void *priv, char *data, u32 len)
{
    ...
    while (hdl->run_flag) {
        ...
        if (wlen) {
            ...
            if ((hdl->recv_size >= hdl->total_size || hdl->recv_size >= SAVE_BUF_SIZE) && hdl->dec_status == 0) {
...
static void dec_server_event_handler(void *priv, int argc, int *argv)
{
    ...
    case AUDIO_SERVER_EVENT_CURR_TIME:
        ...
        os_taskq_post("ui_main_task", 2, UI_MSG_PLEAY_TIME, argv[1]);
        break;
...
static int open_audio_dec(void *priv)
{
    ...
    os_taskq_post("ui_main_task", 2, UI_MSG_PLEAY_ALL_TIME, req.dec.total_time);
...
static void shutdown_audio_play(void *priv)
{
    ...
    /*//关闭播放线程*/
    /*if (hdl->music_task_pid) {*/
    /*thread_kill(hdl->music_task_pid, KILL_WAIT);*/
    /*}*/
...
static void wifi_music_stask(void *priv)
{
    u8 state;
    net_audio_play_test(WIFI_MUSIC_PATH, CONFIG_STORAGE_PATH, 100);
}

void wifi_music_start(void)
{
    puts("wifi_music_start \n\n");
    thread_fork("wifi_music_stask", 11, 1024, 0, 0, wifi_music_stask, NULL);
}

void wifi_music_end(void)
{
    os_taskq_post("net_audio_play_task", 1, SHUTDOWN_CMD);
}
