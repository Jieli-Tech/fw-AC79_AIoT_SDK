...
static void user_get_bt_music_info(u8 type, u32 time, u8 *info, u16 len)
{
    ...
    case 0:
        if (time) {
            ...
            printf("muisc time %d:%d->", player_time_min, player_time_sec);
        }
        break;
    case 1:
        if (len > 0) {
            memcpy(buf, info, len >= sizeof(buf) ? sizeof(buf) - 1 : len);
            printf("bt music title : %s\n", buf);
        }
        deal_type |= BIT(type);
        break;
    case 2:
    ...
    case 3:
        if (len > 0) {
            memcpy(buf, info, len >= sizeof(buf) ? sizeof(buf) - 1 : len);
            printf("bt music album : %s\n", buf);
        }
        deal_type |= BIT(type);
        break;
    case 4:
        memcpy(buf, info, 1);
        printf("bt music num : %d\n", atoi(buf));
        deal_type |= BIT(type);
        break;
    case 5:
        memcpy(buf, info, 1);
        printf("bt music total num : %d\n", atoi(buf));
        deal_type |= BIT(type);
        break;
    case 6:
        if (len > 0) {
            memcpy(buf, info, len >= sizeof(buf) ? sizeof(buf) - 1 : len);
            printf("bt music genre : %s\n", buf);
        }
        deal_type |= BIT(type);
        break;
    case 7:
        ...
        memcpy(buf, info, len);
        printf("bt music total time : %s ms\n", buf);
        deal_type |= BIT(type);
        deal_type = 0;
        break;
    default:
...
#if TCFG_USER_EMITTER_ENABLE && BT_SUPPORT_EMITTER_PAGE_SCAN
...
#if TCFG_USER_EMITTER_ENABLE && BT_SUPPORT_EMITTER_PAGE_SCAN
        if ((get_emitter_curr_channel_state() & A2DP_SRC_CH) == 0) {
            log_i("start to connect a2dp ");
...
static void emitter_auto_connection_deal(void *arg)
{
    ...
    printf("bt emitter auto reconnect remote addr : \n");
